...
void tcp_in(buf_t *buf, uint8_t *src_ip)
{
    ...
    */
    ...
    // TODO
    tcp_hdr_t *hdr = (tcp_hdr_t *)buf->data;
    uint16_t tmp_checksum = hdr->chunksum16;
    hdr->chunksum16 = 0;
    if (tmp_checksum != tcp_checksum(buf, src_ip, net_if_ip)){
        printf("wrong calc checksum : %x\n", tcp_checksum(buf, src_ip, net_if_ip));
        return;
    }
        

    hdr->chunksum16 = tmp_checksum;
    ...
    */
    ...
    // TODO
    uint16_t src_port = swap16(hdr->src_port16), dst_port = swap16(hdr->dst_port16);
    uint32_t seq = swap32(hdr->seq_number32), ack = swap32(hdr->ack_number32);
    ...
    */
    ...
    // TODO
    uint16_t src_wnd_sz = swap16(hdr->window_size16);
    ...
    */
    ...
    if (connect->state == TCP_LISTEN)
    {
        ...
        connect->unack_seq = (uint32_t)time(NULL);
    ...
    */
    ...
    // TODO
    buf_remove_header(buf, TCP_HDR_LEN);
    ...
    case TCP_ESTABLISHED:
        ...
        */
        ...
        // TODO
        // ack means I have ack-ed to ack-1] and I'm expecting [ack
        if (flags.ack && connect->unack_seq < ack && connect->next_seq > ack && buf->len >= ack - connect->unack_seq)
        {
            buf_remove_header(buf, ack - connect->unack_seq);
            connect->unack_seq = ack;
        }
        ...
        */
        ...
        else if (connect->rx_buf->len > 0)
        {
            connect->ack = seq + connect->rx_buf->len;
            tcp_send(&txbuf, connect, tcp_flags_ack);
            (*handler)(connect, TCP_CONN_DATA_RECV);
        }
        uint16_t sz = tcp_write_to_buf(connect, buf);
        if (sz > 0)
        {
